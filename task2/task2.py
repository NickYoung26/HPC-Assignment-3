...
def main() -> None:
    ...
    for p, ((ix0, iy0), label) in enumerate(zip(point_indices, point_labels)):
        rng = np.random.default_rng(seed=1234567 + 1000 * p + rank) #different randomisation for each rank and iteration
        ...
        if rank == 0:
            ...
            sd_boundary = se_boundary * np.sqrt(total_walks)

            g_charge_all[p] = g_charge
            se_charge_all[p] = se_charge
            sd_charge = se_charge * np.sqrt(total_walks)

            g_at_start = g_charge[ix0, iy0]
            se_at_start = se_charge[ix0, iy0]
            std_at_start = se_charge[ix0, iy0] * np.sqrt(total_walks)
            ...
            print(f"  sum G_Laplace over boundary      : {np.sum(g_boundary):.8f}") # sanity check = 1
            print(f"  mean G_Laplace over boundary     : {np.mean(g_boundary):.8e}") # irrelevant
            print(f"  max G_Laplace                    : {np.max(g_boundary):.8e}")
            print(f"  max s.e.(G_Laplace)              : {np.max(se_boundary):.8e}")
            print(f"  max s.d.(G_Laplace)              : {np.max(sd_boundary):.8e}")
            print()
            print(f"  sum G_charge over all sites      : {np.sum(g_charge):.8e}")
            print(f"  mean G_charge over all sites     : {np.mean(g_charge):.8e}")
            print(f"  max G_charge                     : {np.max(g_charge):.8e}")
            print(f"  max s.e.(G_charge)               : {np.max(se_charge):.8e}")
            print(f"  max s.d.(G_charge)               : {np.max(sd_charge):.8e}")
            print()
            print(f"  G_charge at start point          : {g_at_start:.8e}")
            print(f"  s.e.(G_charge at point)          : {se_at_start:.8e}")
            print(f"  s.d.(G_charge at point)          : {np.max(sd_charge):.8e}")

            # Phi Value and Error
            print(f"  Monte Carlo phi                  : {phi_est:.6f} V") # Phi Value
            print(f"  phi from walk average            : {mean:.6f} V")
